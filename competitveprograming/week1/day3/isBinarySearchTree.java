...
public class Solution {
    ...
    public static boolean isBinarySearchTree(BinaryTreeNode root) {
        ...
        int minimum_val=Integer.MIN_VALUE;
        int maximum_val=Integer.MAX_VALUE;
        return isBT(root,minimum_val,maximum_val);
    }
    public static boolean isBT(BinaryTreeNode root,int minimum_val,int maximum_val)
    {
        if(root==null) return true;
        if(root.value<=minimum_val || root.value>maximum_val) return false;
        return isBT(root.left,minimum_val,root.value) && isBT(root.right,root
        .value,maximum_val);
    }
