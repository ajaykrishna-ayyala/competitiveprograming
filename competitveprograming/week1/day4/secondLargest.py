...
def find_second_largest(head):
    ...
    if  (not head or not (head.right or head.left)) :
        raise Exception('cannot be defined')
    return find_second_largest_t(head)



def find_second_largest_t(r, head = None):
    if(not r.right):
        if (r.left):
            return find_largest(r.left)
        return head
    return find_second_largest_t(r.right, r.value)


def find_largest(r):
    if(not r.right):
        return r.value
    return find_largest(r.right)
